...
class TextHighColorPicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget title =  Text(ChsStrings.text1_color, style: theme.theme.textTheme.body1,);
    return new Consumer<ChsThemeModel>(
        builder: (context, model, child) => Container(
          child: !showOnlyCustomTheme ||
            ...
            subtitle: subtitle,
            title: title,
            trailing: CircleColor(color: model.textColorHigh, circleSize: height*0.04,),
            onTap: () async {
              showDialog(
                context: context,
                builder: (BuildContext context) => AlertDialog(
                  title: Text(label),
                  content: ColorPicker(
                      value: model.textColorHigh,
                      onChanged: model.changeTextHighColor,
                    ),
                  ),
              );
            },
