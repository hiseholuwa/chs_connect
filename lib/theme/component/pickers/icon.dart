...
class IconColorPicker extends StatelessWidget {
  const IconColorPicker({
    this.leading,
    this.label = ChsStrings.icon_color,
    this.subtitle,
  ...
  Widget build(BuildContext context) {
    ...
    Widget title =  Text(ChsStrings.icon_color, style: theme.theme.textTheme.body1,);
    return new Consumer<ChsThemeModel>(
        builder: (context, model, child) => Container(
          child: !showOnlyCustomTheme ||
            ...
            subtitle: subtitle,
            title: title,
            trailing: CircleColor(color: model.iconColor, circleSize: height*0.04,),
            onTap: () async {
              showDialog(
                context: context,
                builder: (BuildContext context) => AlertDialog(
                  title: Text(label),
                  content: ColorPicker(
                    value: model.iconColor,
                    onChanged: model.changeIconColor,
                  ),
                ),
              );
            },
